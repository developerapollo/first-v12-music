...
module.exports = {
  name: "play",
  aliases: "p",
  description: "`;play <Şarkı-adı> veya <URL>`",
...
.setColor(COLOR);
    ...
    if (!args.length) {
      ...
      embed.setAuthor("ERROR(Hata Tipi): ;play <URL> or <text>")
    ...
    if (!channel) {
      ...
      embed.setAuthor("Bir Sesli Kanala Girmessen Şarkı Çalmam Knk :/")
      return message.channel.send(embed);
    }
    ...
    if (!videoPattern.test(args[0]) && playlistPattern.test(args[0])) {
      embed.setAuthor("Şimdilik Oynatma Listesini Çalamıyorum")
      return message.channel.send(embed);
    }
    ...
    if (urlcheck) {
      try {
        ...
        if (message.include === "copyright") {
          return message
            .reply("BU VİDEO COPYRİGHT HAKKI NEDENİYLE AÇILAMIYOR -_-")
            .catch(console.error);
        } else {
          console.error(error);
        }
    ...
    if (serverQueue) {
      ...
      embed.setAuthor("Çalan Şarkılara Yenisi Eklendi", client.user.displayAvatarURL())
